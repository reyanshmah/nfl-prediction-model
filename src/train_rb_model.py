"""Train an RB rushing-yards prop model (linear regression + XGBoost).

Mirrors train_player_model.py exactly, for rushing instead of passing.
Loads data/processed/rb_rushing_features.csv (which already includes
is_dome, rest_advantage, spread_line, total_line, and rb_injury_status from
rb_features.py), drops rows with null features, trains on seasons 2022-2024
and evaluates on season 2025 (same season-based split as train_model.py).

Usage:
    python src/train_rb_model.py
"""
...
def load_data() -> pd.DataFrame:
    df = pd.read_csv(RB_FEATURES_PATH)
